...
> **当前版本：v3.0** | **Android App** | 支持双模式运行：前端独立 APK + 前后端联调
>
> :iphone: **可直接在手机上运行** — 项目根目录已提供打包好的安装包 `app-release.apk`，下载到安卓手机安装即可使用。
...
项目根目录已提供预编译的 Release 安装包，可直接下载使用：

| 文件 | 位置 | 说明 |
|------|------|------|
| `app-release.apk` | **项目根目录** `./app-release.apk` | 已签名的 Release 安装包，可直接安装到安卓手机 |
...
1. **下载安装包**：将项目根目录的 `app-release.apk` 传输到安卓手机
2. **安装应用**：在手机上点击 APK 文件，允许"未知来源"安装
3. **打开即用**：安装完成后打开 App，**无需任何配置**
4. **确保联网**：使用前请开启 WiFi 或移动数据（AI 对话需联网）
5. **切换模式（可选）**：首次启动可在设置页配置后端地址，切换为前后端联调模式（模式 B）
